...
import TSP_utils
TSP_datasets = {'TSPLIB': TSP_utils.datasets['TSPLIB']}
...
def select_next_node(current_node: int, unvisited_nodes: list, distance_matrix: np.ndarray) -> int:
    """Heuristic function to return the next node to visit by random weights in order to achieve overall lowest minimum distance travelled.
    ...
    """
    """Improved version of `select_next_node_v0`."""

    num_nodes = len(unvisited_nodes)
    scores = []

    for node in unvisited_nodes:
        distances = [distance_matrix[node-1, i-1] for i in unvisited_nodes if i != node]
        if (len(distances) == 0):
            score = 0
        else:
            average_distance = np.mean(distances)
            std_dev_distance = np.std(distances)
            score = (0.5 * distance_matrix[current_node-1, node-1]) - (0.3 * average_distance) + (0.2 * std_dev_distance)
        scores.append(score)

    min_score = min(scores)
    min_score_index = scores.index(min_score)
    return unvisited_nodes[min_score_index]
'''
...
if __name__ == '__main__':
    ...
    config = config.Config(samples_per_prompt=6, evaluate_timeout_seconds=40)
